...
{
    size_t free_space = CBFreeSpace(c_buffer);
    ...
    memcpy((unsigned char*)c_buffer + offset, (char*)src + remain_bytes, bytes - remain_bytes);
    c_buffer->size += bytes;
...
ssize_t CBRead(circular_buffer_t* c_buffer, void* dst, size_t bytes)
{
    ...
    memcpy((char*)dst + remain_bytes, (unsigned char*)c_buffer + offset, bytes - remain_bytes);
