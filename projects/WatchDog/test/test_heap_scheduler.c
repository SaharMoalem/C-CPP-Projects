...
void TestScheduler(void)
{
	...
	TEST("Scheduler Stopped", stat, SCHED_STOPPED);
	...
	stat = SchedulerRun(sch);
	TEST("Scheduler finished running", stat, SCHED_SUCCESS);
	TEST("Scheduler is empty", SchedulerIsEmpty(sch), 1);
	...
	stat = SchedulerRun(sch);
	TEST("Scheduler status while running test", stat, SCHED_SUCCESS);
